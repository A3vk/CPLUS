...
SubtractCommand::SubtractCommand(std::shared_ptr<Stack<std::string>> stack) : m_Stack(stack) {}

bool SubtractCommand::IsCommand(std::string& line)
{
	return line.rfind(syntax, 0) != std::string::npos;
}

void SubtractCommand::Execute(std::string& line)
{
	if (m_Stack->size() < numberOfParameters)
	{
		throw InterpreterException("Subtract - The sub command requires to have two values on the stack");
	}
